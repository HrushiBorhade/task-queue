...
export function createWebhookWorker() {
  const config = QUEUE_CONFIGS.webhook_processing;
  ...
  return new Worker<TaskJobPayload>(
    ...
    async (job) => {
      ...
      try {
        ...
        await tracker.broadcastStep("Delivering webhook");
        tracker.updateProgress(50);

        await new Promise((r) => setTimeout(r, 200));
        tracker.updateProgress(95, "Webhook delivered");

        const result = `Webhook delivered to: ${input.prompt}`;
